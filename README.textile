...
The groovyx.osgi provides two packages to make working with OSGi easier. Additionally they are used by the "Grails OSGi plugin":http://github.com/jetztgradnet/grails-osgi-plugin/ from version 0.3 on.

h3. groovx.osgi

This package contains helper classes to make OSGi programming easier. This module is inspired by "ScalaModules":http://github.com/weiglewilczek/scalamodules/wiki.
...
bc.	import grooxy.osgi.runtime.OsgiRuntimeBuilder
...
directory and is called @dropin@ or @load@ or @pickup@.
...
*Note*: an external runtime csn neither be started nor stopped; the @BundleContext@ is not available.

h3. Installing bundles from files, URLs and Maven repositories
